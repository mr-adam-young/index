...
<?php
if ($_GET['job'] == 'new') {
	echo '<form action="'.$_ENV['SITE_ROOT'].'/?job='.$job['ID'].'&action=new" method="post">';
} else {
	echo '<form action="'.$_ENV['SITE_ROOT'].'/?job='.$job['ID'].'&action=edit" method="post">';
}
...
<script language="javascript">
	$(".invoice_submit").click(function(){
     var localTarget = $(this);
		$.post("processing.php?action=invoice", 
		{
			...
			job_id: $(this).siblings("#job_id").val(),
			cost: $(this).siblings("#cost").val(),
			description: $(this).siblings("#description").val(),
			vendor: $(this).siblings("#vendor").val(),
		},
		function(data) { 
      $("#invoice_table").append("<tr id=\"" + data[0]["ID"] + "\"><td>" + localTarget.siblings("#cost").val() + "</td><td>" + localTarget.siblings("#description").val() + "</td><td>" + localTarget.siblings("#vendor").val() + "</td><td><button type=\"button\" class=\"remove_invoice\">Remove</button></td></tr>");
      localTarget.siblings("#cost").val("");
      localTarget.siblings("#job_id").val("");
      localTarget.siblings("#description").val("");
      localTarget.siblings("#vendor").val("");
		}); 
	});
	...
	$(".estimate_submit").click(function(){
     var localTarget = $(this);
      ...
      localTarget.siblings("#estimate_hours").val("");
		}); 
	});
	...
	$(".costing_estimates_submit").click(function(){
     var localTarget = $(this);
		$.post("processing.php", 
		{
			...
			costing_estimates_job_id: $(this).siblings("#costing_estimates_job_id").val(),
			costing_estimates_cost: $(this).siblings("#costing_estimates_cost").val(),
			costing_estimates_description: $(this).siblings("#costing_estimates_description").val(),
			costing_estimates_vendor: $(this).siblings("#costing_estimates_vendor").val(),
		},
		function(data) { 
      $("#costing_estimates_table").append("<tr id=\"" + data[0]["ID"] + "\"><td>" + localTarget.siblings("#costing_estimates_cost").val() + "</td><td>" + localTarget.siblings("#costing_estimates_description").val() + "</td><td>" + localTarget.siblings("#costing_estimates_vendor").val() + "</td><td><button type=\"button\" class=\"remove_invoice\">Remove</button></td></tr>");
      localTarget.siblings("#costing_estimates_cost").val("");
      localTarget.siblings("#costing_estimates_description").val("");
      localTarget.siblings("#costing_estimates_vendor").val("");
		}); 
	});
